defmodule EphemeralShare.PeerChannel do
  ...
  def join("peer:" <> peer_id, _auth_msg, socket) do
    Logger.debug("[PeerChannel] join peer:#{peer_id}")
  ...
  def handle_in("connect", %{"peer_id" => peer_id, "sender_id" => sender_id}, socket) do
    Logger.debug("[PeerChannel] Connect Request #{peer_id}, #{sender_id}")

    case send_to_peer(peer_id, "peer_connect", %{peer_id: sender_id}) do
      {:error, _} ->
        Logger.error("[PeerChannel] Error sending to peer #{peer_id}")
        push(socket, "error_connect", %{id: peer_id})

      :ok ->
        Logger.debug("[PeerChannel] Sent peer connect request to #{peer_id} from #{sender_id}")

      x ->
        Logger.error("[PeerChannel] Got unexpected peer connect #{x}")
    end
  ...
  def handle_in(
        "offer",
        %{"offer" => offer, "peer_id" => peer_id, "sender_id" => sender_id},
        socket
      ) do
  ...
  def handle_in(
        "answer",
        %{"answer" => answer, "peer_id" => peer_id, "sender_id" => sender_id},
        socket
      ) do
  ...
  defp send_to_peer(peer_id, event, msg) do
    case PeerManager.exists?(peer_id) do
      false ->
        Logger.error("[PeerChannel] Peer #{peer_id} does not exist")
        {:error, %{reason: "No such client exists"}}

      true ->
        Logger.info("[PeerChannel] Broadcasting to peer #{peer_id}")
        EphemeralShareWeb.Endpoint.broadcast("peer:" <> peer_id, event, msg)
    end
  end
