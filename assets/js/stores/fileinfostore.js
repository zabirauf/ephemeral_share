...
export class FileInfoStore extends EventEmitter {
  ...
  constructor(peerComm, isInitiator) {
    ...
    if (this.isInitiator) {
      this.peerComm.addEventListener(
        PeerCommunicationConstants.PEER_CONNECTED,
  ...
  initialize() {
    ...
    FileTransferManager.instance().addOnFileDownloadCompleteListener(
      this.onFileDownloadComplete.bind(this)
    );
    FileTransferManager.instance().addOnFileDownloadProgressListener(
      this.onFileDownloadProgress.bind(this)
    );

    // Listeners for upload progress and complete
    FileTransferManager.instance().addOnFileUploadCompleteListener(
      this.onFileUploadComplete.bind(this)
    );
    FileTransferManager.instance().addOnFileUploadProgressListener(
